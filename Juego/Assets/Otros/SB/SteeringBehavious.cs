...
{
	static public Vector3 Seek(SBAgent agent, Vector3 target, float range = 99999)
	{
		
		// cálculo del vector deseado
		Vector3 desired = Vector3.zero;

		Vector3 difference = (target - agent.transform.position);
	...
	static public Vector3 Flee(SBAgent agent, Vector3 target, float range = 99999)
	{
		// cálculo del vector deseado
		Vector3 desired = Vector3.zero;
	...
	static public Vector3 Separate(SBAgent agent, List<GameObject> agentsToAvoid, float range){
		...
		for(int i = 0; i < agentsToAvoid.Count; i++){
			steer += Flee(agent, agentsToAvoid[i].transform.position,range); 
		}
		return steer;
	}
}